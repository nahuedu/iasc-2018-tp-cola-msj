...
module.exports = (msg, socket, statusManager) => {
  ...
  if (topic) {
    ...
    if (topic.tipoCola == 'publicar_suscribir') {
      process.send({ tipo: "createQueue", topicTitle: topic.topic, tipoCola: topic.tipoCola, idConsumer: consumer.id });
      socket.on('disconnect', () => {
        process.send({ tipo: "deleteQueue", topicTitle: topic.topic, tipoCola: topic.tipoCola, idConsumer: consumer.id });
      });
    }
